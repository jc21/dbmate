...
func newTestDB(t *testing.T, u *url.URL, slaveDatabases []*url.URL) *dbmate.DB {
	var err error
	...
	db := dbmate.New(u, slaveDatabases)
...
func TestNew(t *testing.T) {
	db := dbmate.New(dbutil.MustParseURL("foo:test"), nil)
...
func TestGetDriver(t *testing.T) {
	...
	t.Run("missing schema", func(t *testing.T) {
		db := dbmate.New(dbutil.MustParseURL("//hi"), nil)
		drv, err := db.GetDriver()
		require.Nil(t, drv)
		require.EqualError(t, err, "invalid url, have you set your --url flag or DATABASE_URL environment variable?")
	})
	...
	t.Run("invalid driver", func(t *testing.T) {
		db := dbmate.New(dbutil.MustParseURL("foo://bar"), nil)
		drv, err := db.GetDriver()
...
func testWaitBefore(t *testing.T, verbose bool) {
	u := dbutil.MustParseURL(os.Getenv("POSTGRES_TEST_URL"))
	s := dbutil.MustParseURLs(os.Getenv("POSTGRES_TEST_URL_SLAVE"))
	db := newTestDB(t, u, s)
	db.Verbose = verbose
	db.WaitBefore = true
...
func TestWaitBeforeVerbose(t *testing.T) {
	...
	require.Contains(t, output,
		`Applying: 20151129054053_test_migration.sql (master: true, slaves: 1)
Rows affected: 0
- Applied to slave #1
Rows affected: 1
- Applied to master
Applying: 20200227231541_test_posts.sql (master: true, slaves: 1)
- Applied to slave #1
Rows affected: 0
- Applied to master`)
	require.Contains(t, output,
...
func TestMigrateWithSlave(t *testing.T) {
	t.Run("Postgres with Slave", func(t *testing.T) {
		u := dbutil.MustParseURL(os.Getenv("POSTGRES_TEST_URL"))
		s := dbutil.MustParseURLs(os.Getenv("POSTGRES_TEST_URL_SLAVE"))
		db := newTestDB(t, u, s)
		drv, err := db.GetDriver()
		...
		// verify slave
		for _, sDrv := range sDrvs {
			sqlDBSlave, err := sDrv.Open()
			require.NoError(t, err)
			defer dbutil.MustClose(sqlDBSlave)

			count := 0
			err = sqlDBSlave.QueryRow(`select count(*) from schema_migrations
				where version = '20151129054053'`).Scan(&count)
			require.NoError(t, err)
			require.Equal(t, 1, count)

			// There should be no users as the slave code only created the table
			err = sqlDBSlave.QueryRow("select count(*) from users").Scan(&count)
			require.NoError(t, err)
